	...
	public static boolean isValid(String str) {
		...
		for(int i = 0; i < str.length(); i++ ) {
			...
			//	올바르면, 문자 하나를 팝합니다.
				...
				//	1. 만약 스택이 비어 있다면(top == -1), 이 뜻은 열기 괄호수보다 닫기 괄호수가
				//	많아졌다는 의미이므로 올바르지 않은 괄호쌍이 됩니다.
				//	스택에 최상위 문자가 현재 검사하는 문자와 다른 종료라면, 역시 올바르지 않은 
				//	괄호쌍이 됩니다.
				if(top == -1 || stack[top] != '(') return false;
				//	2. 스택에서 최상위 아이템을 제가합니다.
				top--;
			}
			else if(ch == '}') {
				//	1. 만약 스택이 비어 있다면(top == -1), 이 뜻은 열기 괄호수보다 닫기 괄호수가
		...
		//	3. 스택이 비어있다면(top == -1) true 반환, 그렇지 않으면 false를 반환합니다.
